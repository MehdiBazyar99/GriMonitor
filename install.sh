...
uninstall() {
    ...
    echo "GriMonitor has been uninstalled."
}

# Function to install the script
install() {
    # Remove old version of the script, virtual environment, wrapper script, and configuration file if they exist
    echo "Removing old version of the script, virtual environment, wrapper script, and configuration file if they exist..."
    uninstall

    # Download the new Python script
    echo "Downloading uptime_monitor.py from GitHub..."
    curl -s -o "$SCRIPT_NAME" "$REPO_URL"
...
# Check if the script is being uninstalled or installed
if [ "$1" == "uninstall" ]; then
    uninstall
else
    install
